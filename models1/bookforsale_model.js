...
var bfs={
    ...
    editBookforsalewithoutfile:function(data,callback){
        ...
        console.log(data);
        return db.query('update bookforsale_tbl set book_isbn=?,book_title=?,book_author=?,book_price=?,book_publisher=?,book_ratings=?,fk_category_id=? where book_id=?',[data.book_isbn,data.book_title,data.book_author,data.book_price,data.book_publisher,data.book_ratings,data.fk_category_id,data.book_id],callback);
    },
